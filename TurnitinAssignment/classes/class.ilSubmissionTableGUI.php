       	...
       	if (!empty($a_set["title"]))
       	{
       		...
       		switch ($a_set["similarityScore"])
	        {
	        	...
	        	case "-1":
	        		...
	        		break;
	        	default:
                    if ($a_set["report_score_to_show"])
                    {
                        $score = $a_set["report_score_to_show"];
                    } else {
                        $score = $a_set["overlap"];
                        $a_set["report_score_lng_overlay"] = '';
                    }

	        		$this->tpl->setVariable("VAL_ORIG_REPORT_SCORE", $score);
            ...
            if (is_array($a_set["student_responses"]))
			{
